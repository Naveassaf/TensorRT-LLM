...
class Gemma3DecoderLayer(DecoderLayer):
    ...
    def __init__(
        ...
        is_sliding = bool((layer_idx + 1) % pretrained_config.sliding_window_pattern)
        ...
        self.input_layernorm = RMSNorm(hidden_size=pretrained_config.hidden_size,
                                       eps=pretrained_config.rms_norm_eps,
                                       dtype=pretrained_config.torch_dtype)
        self.post_attention_layernorm = RMSNorm(hidden_size=pretrained_config.hidden_size,
                                                eps=pretrained_config.rms_norm_eps,
                                                dtype=pretrained_config.torch_dtype)
        self.pre_feedforward_layernorm = RMSNorm(hidden_size=pretrained_config.hidden_size,
                                                 eps=pretrained_config.rms_norm_eps,
                                                 dtype=pretrained_config.torch_dtype)
                ...
                if names[-1] in params_map:
                    ...
                    for new_name in params_map[names[-1]]:
                        ...
                        if new_name in ['k_proj', 'v_proj']:
                            # module.quant_config.layer_quant_mode.is_int4_weight_only_per_group() or module.quant_config.layer_quant_mode.has_nvfp4()
                            fw = {
                                k:
                                duplicate_kv_weight(
                                    weight=v[:],
                                    head_dim=head_dim,
                                    tensor_parallel_size=tp_size, factor=2)
